...
public class CacheService {
    ...
    public void analyzePutResourceInCache(int position, Long idRequest, Object res){
        ...
        String requestGroup = idRequest.toString().substring(0, 1);
        for (long idInCache: cacheStorage.keySet().stream().filter(cacheVal -> cacheVal.toString().substring(0, 1).equals(requestGroup)).toList()) {
